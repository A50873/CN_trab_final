...
import cn2026server.services.functional.FunctionalServiceImpl;
import cn2026server.services.scaling.ScalingServiceImpl;
...
public class GrpcServer {
    ...
    static void main(String[] args) throws InterruptedException, IOException {
        ...
        try {
            ...
            // Create processing labels instance scaler if MIG name is provided
            String processingMigName = "cn2026-labels-mig";
            try {
                processingInstanceScaler = new GcpInstanceGroupScaler(projectId, zone, processingMigName);
                System.out.println("Initialized Processing Labels Instance scaler for MIG: " + processingMigName);
            } catch (Exception e) {
                System.err.println("Failed to initialize Processing Labels Instance scaler: " + e.getMessage());
            }
        } catch (Exception e) {
